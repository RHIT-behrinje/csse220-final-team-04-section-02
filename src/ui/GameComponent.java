...
////		 timer.start();
		...
		timer = new Timer(70, e -> {
			...
			if (player.boundingBox().intersects(exit.boundingBox()) && level < 3) {
				level++;
				loadLevel(level);
			...
			for (Zombie z : zombie) {
				...
				if (z.boundingBox().intersects(player.boundingBox())) {
					player.handleKilled();
				}
	...
	void catapult() {
		for (int i = 0; i < zombie.size(); i++) {
			...
			if (player.x - z1.x == GameModel.tileSize && player.y == z1.y) {
				if (wallTest(z1.x - GameModel.tileSize * 2, z1.y)) {
					z1.launchnx();
					z1.launchnx();
				} else if (wallTest(z1.x - GameModel.tileSize, z1.y)) {
					z1.launchnx();
				}
			}
			...
			if (player.y - z1.y == GameModel.tileSize && player.x == z1.x) {
				// z1.moveny();
				if (wallTest(z1.x, z1.y - GameModel.tileSize * 2)) {
					z1.launchny();
					z1.launchny();
				} else if (wallTest(z1.x, z1.y - GameModel.tileSize)) {
					z1.launchny();
				}
			}
